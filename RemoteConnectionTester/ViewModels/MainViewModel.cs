...
using System.Text;
using static RemoteConnectionTester.PingActor;

namespace RemoteConnectionTester.ViewModels;

public class MainViewModel : ViewModelBase
{
    private IActorRef? pingActor;
    ...
    public MainViewModel()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                string networkAccessMessage = $"NetworkAccess: {accessType}";
                sb.Append(networkAccessMessage);
            }
            catch (Exception ex)
            {
            ...
            System.Diagnostics.Debug.WriteLine(sb.ToString(), "[INFO]");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine(ex);
    ...
    public string? AndroidIp
    {
        get { return this.androidIp; }
        set { this.RaiseAndSetIfChanged(ref this.androidIp, value); }
    }

    private string? androidPort;
    public string? AndroidPort
    {
        get { return this.androidPort; }
        set { this.RaiseAndSetIfChanged(ref this.androidPort, value); }
    }
    ...
    public void CreateActorSystem()
    {
        try
        {
            ...
            var actorSystemName = GuidString();

            this.actorSystem?.Terminate();
            this.actorSystem = ActorSystem.Create(actorSystemName, hocon);

            var props = Props.Create(() => new PingActor(ServerIp!, ServerPort!));
            this.pingActor = this.actorSystem.ActorOf(props, "ping-actor");
        }
        catch (Exception ex)
        {
            Logs.Add(ex.ToString());
    ...
    {
        if (this.actorSystem is null)
        {
            Logs.Add("No Actor System found, create one first.");
            return;
        }

        this.pingActor.Tell(new PingRequest());
    }
